...
if ! options=$(getopt -o VNnfupthdC:lL: -l virtual-env,no-virtual-env,no-site-packages,force,update,smoke,parallel,help,debug,config:,logging,logging-config: -- "$@")
then
    # parse error
    usage
    exit 1
...
# Ready for vsm env or vsm env is ready
